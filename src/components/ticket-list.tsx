...
import { Plus, Bug, Lightbulb, Sparkles, MessageSquare, Trash2, ExternalLink } from "lucide-react";
...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  RESOLVED: { label: "Gelöst", color: "bg-emerald-50 text-emerald-700" },
...
export function TicketList() {
  ...
  async function handleDelete(ticket: Ticket) {
    if (!confirm(`"${ticket.title}" wirklich löschen?`)) return;
    try {
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Priorität</th>
